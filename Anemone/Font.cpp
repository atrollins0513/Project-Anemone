...
namespace Anemone
{
	namespace Font
	{
		...
		const FontCharInfo& FontLoader::getCharacter(AE_UINT index)
		{
		...
		void FontManager::init()
		{
			...
			fs.AddLine("AE_FLOAT distance = 1.0 - texture(texture_id, fragTexCoord).a; AE_FLOAT alpha = 1.0 - smoothstep(width, width+edge, distance); color = vec4(fragColor, alpha);");
		...
		void FontManager::update()
		{
			...
			std::vector<ae::vec3> color;
			std::vector<AE_UINT> indices;
			AE_UINT index_offset = 0;
			index_count = 0;
			...
			for (auto t : texts)
			{
				if (t->display)
				{
					ae::vec3 start_position = t->position;
					ae::vec3 position = t->position;
					AE_UINT xadvance = 0;
					for (auto s : t->text)
					{
